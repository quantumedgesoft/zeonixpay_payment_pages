...
//   paymentMethod: string; // e.g. "bkash-agent" | "bkash-personal" | "bkash-merchant"
// };
...
//   const manual_payment_url = `${base}/get-payment/${paymentMethod.split("-")[0]}-payment/?method=${paymentMethod}&invoice_payment_id=${merchantInvoiceId}`;
...
//       await onVerifyTrx?.(trxId.trim());

//       // Redirect to callback-redirect with invoice id + status
//       const status = json?.status === true ? "success" : "failed";
//       router.push(
//         `/payment/callback-redirect?invoice_payment_id=${encodeURIComponent(
//           json?.invoice_payment_id ?? merchantInvoiceId
//         )}&status=${status}`
//       );
//     } catch (e) {
//       console.error(e);
...
//       try {
//         const res = await fetch(manual_payment_url, { cache: "no-store" });
...
//           <Image src={merchantLogoSrc} alt="Gateway" width={50} height={50} className="object-contain" />
//         </div>
//       </div>
...
//       {/* Instruction card */}
//       <div className={`mx-6 my-6 rounded-xl text-white`} style={{ backgroundColor: providerColor }}>
...
//             <span className="font-bold bg-white/20 rounded px-2 py-0.5">
//               {invoiceData?.data?.Number ?? receiverMsisdn}
//             </span>
//             <button
//               onClick={copyNumber}
...
//           className="flex-1 rounded-xl px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:opacity-90 disabled:opacity-60"
//         >
...
export default function BkashAgentFlow({
  ...
  const actionText = paymentMethod.includes("personal") ? "Send Money" : "Cash Out";
  console.log(receiverMsisdn);

  const copyNumber = async () => {
    const numberToCopy = invoiceData?.data?.Number ?? "";
    if (!numberToCopy) {
    ...
    try {
      ...
      toast.success(`Copied: ${numberToCopy}`);
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl ring-1 ring-black/5 overflow-hidden mx-auto">
      ...
      <div className="flex items-center gap-4 px-6 pt-5">
        <div className="flex-1 rounded-xl border border-slate-200 p-3">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-violet-100 overflow-hidden">
              {brandLogo ? (
                <Image
                  src={brandLogo}
                  alt="brand-logo"
                ...
                <span className="text-violet-700 font-semibold">৳</span>
              )}
            </div>
